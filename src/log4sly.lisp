...
(in-package #:log4cl.slynk)
(log:package-options :shortest-nickname nil)

;; Accessors for sly (DSPEC LOC) 
(defun sly-loc-type (loc) (first (first loc)))
(defun sly-loc-name (loc) (second (first loc)))

(defun sly-loc-type-figurer (type)
  "Parse Sly DSPEC type, and figure if its a DEFUN, DEFMETHOD or
...
(defun sly-loc-defun-p (loc) (eq (sly-loc-type loc) 'slynk::defun))
(defun sly-loc-defvar-p (loc) (eq (sly-loc-type loc) 'slynk::defvar))
...
return it, otherwise return all of them, so Sly will pop the xref
window"
  ...
  (let* ((tmp (mapcar (lambda (x)
                        (cons (sly-loc-type-figurer (sly-loc-type x))
                              x))
                      definitions))
         (defuns (progn
                   (log:debug tmp)
    ...
    (when (plusp (length defuns)) 
      (log:warn "SLY found more then one DEFUN for ~S" (first categories))
...
(defun install (&key force
  ...
  (let ((*print-pretty* t)
    ...
    (flet ((print-dot-emacs-message (filename)
             ...
             (format t "(global-log4sly-mode 1)~%")
             (format t "------------------------------------------------------~%"))
           ...
           (generate-the-file (filename dir)
             (with-open-file (out filename
               ...
               (format out ";; load Log4sly support~%~%")
               (format out "(add-to-list 'load-path ~S)~%"
                       (homepath dir))
               (format out "(require 'log4sly)~%"))
                 ...
                 (let ((directory (directory-namestring destination-directory)))
                   (if (pathname-match-p destination-directory *quicklisp-directory*)
                       ...
                       \"~A\" does not exist. Try ~(~:<~A ~A:~A ~S~:@>~) ~:@>"
                               (homepath directory)
                               `(setf :log4sly *quicklisp-directory*
      ...
      (check-directories)
      (let* ((filename (make-pathname :name "log4sly-setup" :type "el"
